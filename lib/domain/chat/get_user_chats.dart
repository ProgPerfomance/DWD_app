...
class GetUserChats extends ChangeNotifier {
  ...
  for(int i =0; i < data.length; i++) {
    chats.add(GetChatModel(avatar: '', cid: data[i]['cid'], last_message: data[i]['last_message'], subject_id: data[i]['subject_id'], subject_name: data[i]['subject_name'], uid_opponent: data[i]['uid_opponent'], oponent_name: data[i]['opponent_name'], ));
  }
notifyListeners();
